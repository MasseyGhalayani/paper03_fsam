...
def main():
    ...
    if args.optimizer == 'SAM':
        ...
        optimizer = torch.optim.SGD(momentum=args.momentum,weight_decay=args.weight_decay)
        ...
        optimizer = FriendlySAM(model.parameters(), base_optimizer, rho=args.rho, sigma=args.sigma, lmbda=args.lmbda,
                                adaptive=0, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay, \
                                nesterov=False)
    elif args.optimizer == 'FriendlySAM_adamw':
        base_optimizer = torch.optim.AdamW
        optimizer = FriendlySAM(model.parameters(), base_optimizer, rho=args.rho, sigma=args.sigma, lmbda=args.lmbda,
                                adaptive=0, lr=args.lr, weight_decay=args.weight_decay)

    print(optimizer)
    if args.schedule == 'step':
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer.base_optimizer, milestones=[60, 120, 160],
                                                            gamma=0.2, last_epoch=args.start_epoch - 1)
